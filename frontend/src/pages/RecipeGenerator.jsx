...
const RecipeGenerator = () => {
    ...
    useEffect(() => {
        const fetchUserPreferences = async () => {
            try {
                const response = await api.get('/users/profile'); // fix: missing =
                const preferences = response.data.data.preferences;

                if (preferences) {
                    // auto fill dietary preferences
                    if (preferences.dietary_restrictions && preferences.dietary_restrictions.length > 0) {
                        setDietaryRestrictions(preferences.dietary_restrictions);
                    }
                    // auto fill preferred cuisines - use first one if multiple
        ...
        fetchUserPreferences(); // fix: was never called
    }, []);
    ...
    const handleSaveRecipe = async () => { // fix: was handleSavedRecipe, renamed to match JSX
        ...
        try {
            await api.post('/recipes', {
                ...
                cuisine_type: generatedRecipe.cuisineType, // fix: was cuisine_Type
                                ...
                                {generatedRecipe.nutrition && (
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                                            ...
                                            <NutritionBadge label="Carbs" value={generatedRecipe.nutrition.carbohydrates} unit="g" /> {/* fix: was .carbs */}
